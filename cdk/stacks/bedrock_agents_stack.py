...
class OscarAgentsStack(Stack):
    ...
    def _create_jenkins_agent(self) -> tuple[bedrock.CfnAgent.AgentCollaboratorProperty, bedrock.CfnAgentAlias]:
        ...
        jenkins_action_group = bedrock.CfnAgent.AgentActionGroupProperty(
            ...
            action_group_executor=bedrock.CfnAgent.ActionGroupExecutorProperty(
                lambda_=self.lambda_stack.lambda_functions[
                    self.lambda_stack.get_jenkins_agent_lambda_function_name(self.env_name)].function_arn,
            ),
        ...
        collaborator_specs = bedrock.CfnAgent.AgentCollaboratorProperty(
            ...
            collaboration_instruction="This is Jenkins-Specialist agent specializes in Jenkins job operations, build execution, and job parameter validation. It can execute Docker security scans, build jobs, release promotion pipelines (among other jobs), and provide comprehensive job information. Collaborate with this Jenkins-Specialist for all Jenkins-related operations and job execution requests.Only call the trigger_bot function after user confirmation --> So send queries to the jenkins-specialist regarding triggering the job explicitly only when confirmation has been received.",
    ...
    def _create_metrics_build_agent(self) -> tuple[bedrock.CfnAgent.AgentCollaboratorProperty, bedrock.CfnAgentAlias]:
        metrics_build_action_group = bedrock.CfnAgent.AgentActionGroupProperty(
            ...
            action_group_state="ENABLED",
            action_group_executor=bedrock.CfnAgent.ActionGroupExecutorProperty(
                lambda_=self.lambda_stack.lambda_functions[self.lambda_stack.get_metrics_agent_lambda_function_name(self.env_name)].function_arn,
            ),
            function_schema=bedrock.CfnAgent.FunctionSchemaProperty(
                functions=[
                    bedrock.CfnAgent.FunctionProperty(
        ...
        metrics_build_agent = bedrock.CfnAgent(
            ...
            instruction="""You are a Build Specialist for the OpenSearch project.
        ...
        collaborator_specs = bedrock.CfnAgent.AgentCollaboratorProperty(
            ...
            collaboration_instruction="This Build-Metrics-Specialist agent specializes in build metrics, distribution build analysis, and build pipeline performance. It can analyze build failures, success rates, and component-specific build issues across different versions and time ranges. Collaborate with this Build-Metrics-Specialist for dynamic/analytical queries regarding Build Metrics.",
    ...
    def _create_metrics_test_agent(self) -> tuple[bedrock.CfnAgent.AgentCollaboratorProperty, bedrock.CfnAgentAlias]:
        ...
        collaborator_specs = bedrock.CfnAgent.AgentCollaboratorProperty(
            ...
            collaboration_instruction="This Test-Metrics-Specialist agent specializes in integration test failures, RC-based analysis, and component testing patterns. It can analyze test failures across different platforms, architectures, and distributions. You provide detailed failure analysis with test reports and build URLs for debugging. Collaborate with this Test-Metrics-Specialist for dynamic/analytical queries regarding Test Metrics.",
...
"""
    ...
    def _create_supervisor_agent(self, collaborators: List[bedrock.CfnAgent.AgentCollaboratorProperty]) -> None:
        ...
        """
        # Create communication action group
        communication_action_group = bedrock.CfnAgent.AgentActionGroupProperty(
            ...
            action_group_state="ENABLED",
            action_group_executor=bedrock.CfnAgent.ActionGroupExecutorProperty(
                lambda_=self.lambda_stack.lambda_functions[self.lambda_stack.get_communication_handler_lambda_function_name(self.env_name)].function_arn,
            ),
            function_schema=bedrock.CfnAgent.FunctionSchemaProperty(
                functions=[
                    bedrock.CfnAgent.FunctionProperty(
        ...
        INTELLIGENT ROUTING CAPABILITIES
            ...
            OpenSearch configuration, installation, APIs, commands & information to build and test, and implementation-level code.
            ...
            JENKINS QUERIES → Specialist Collaborators
            ...
            For communication orchestration queries, NEVER send the message to the channel UNTIL AFTER you have acknowledged direct confirmation from the user.
            ...
            **STEP 3: DATA COLLECTION**
            ...
            For "missing release notes" → Query Release-Readiness-Metrics-Specialist: "What components are missing release notes for version X.X.X?"
            
            For "criteria not met" → Query Release-Readiness-Metrics-Specialist: "What entrance criteria are not met for version X.X.X?"
            
            For "build status" → Query Build-Metrics-Specialist for build status
            
            For "integration tests" → Query Test-Metrics-Specialist for coverage data
            ...
            **ENFORCEMENT RULES:**
            ...
            If the user does not intend to execute a Jenkins job, abort this workflow. If it is regarding running ANY workflow or JOB, it must go through this workflow.
            
            ### **STEP 2: JOB DISCOVERY AND VALIDATION**
            Route to the Jenkins-Specialist collaborator and use its functions to gather job information:
            
            **For specific job requests:**
            - Call Jenkins-Specialist `get_job_info` with the job name to retrieve job description, requirements, parameters, and Jenkins job URL
            
            **For general requests (e.g., "scan nginx:latest"):**
            - Call Jenkins-Specialist `list_jobs` to discover available jobs
            - Match user intent to appropriate job type
            - Call Jenkins-Specialist `get_job_info` for the matched job
            
            **For unknown job names:**
            - Call Jenkins-Specialist `list_jobs` to show available options
            ...
            **Once confirmed:**
            1. Call Jenkins-Specialist `trigger_job` with:
            ...
            6. **ALWAYS route to Jenkins-Specialist collaborator for all Jenkins functions**
        """
        )
